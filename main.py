...
text_size = [2 * i for i in range(50)]
...
def convert_txtfile_to_pdf(file_path, output_path, selected_font, selected_size):
   pdf = FPDF()
   pdf.add_page()
   pdf.set_font(selected_font, size=selected_size)
...
def select_multiple_files(action):
   ...
   if action == "convert":
       for file_path in file_paths:
           ...
           selected_size = int(text_size_combobox.get())
           output_path = filedialog.askdirectory(title="Select Output Directory")
           convert_txtfile_to_pdf(file_path, output_path, selected_font, selected_size)
   elif action == "merge":
       if len(file_paths) <= 1:
...
def main():
    ...
    global text_size_combobox
    text_size_combobox = ttk.Combobox(convert_frame, values=text_size, state="readonly")
    text_size_combobox.current(7)
    text_size_combobox.pack(pady=5)
